...
export default function PlanningToolsPage() {
  ...
  const tasks = useQuery(api.events.getTasks, { eventId: params.id });
  const budgetItems = useQuery(api.events.getBudgetItems, { eventId: params.id });
  const bringItems = useQuery(api.events.getBringItems, { eventId: params.id });
  const timelineItems = useQuery(api.events.getTimelineItems, { eventId: params.id });
  ...
  const toggleTimelineItemCompleted = useMutation(api.events.toggleTimelineItemCompleted);
        ...
        <div className="space-y-3 animate-slide-up">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-on-surface-variant">
              <span className="font-bold text-primary">{completedTasks}</span> / {tasks.length} done
            </p>
          </div>
          ...
          {event.isHost && (
            <GlassCard className="p-4 space-y-3">
              <input
                ...
                value={taskForm.text}
                onChange={(event) =>
                  setTaskForm((current) => ({ ...current, text: event.target.value }))
                }
              />
              <input
                className="input-field"
              ...
              <GlassCard key={task._id} className="p-4">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() =>
                      event.isHost &&
                      toggleTaskMutation({ id: task._id, completed: !task.completed })
                    }
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                      task.completed ? "bg-primary border-primary" : "border-outline-variant/40"
        ...
        <div className="space-y-4 animate-slide-up">
          <GlassCard className="p-5">
            ...
            <div className="h-2 bg-surface-container-highest rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: totalBudget > 0 ? `${(paidBudget / totalBudget) * 100}%` : "0%",
                }}
              />
            </div>
          </GlassCard>
          ...
          {budgetItems.length === 0 ? (
            ...
            budgetItems.map((item) => (
              <GlassCard key={item._id} className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button
      ...
      {activeTab === "bring" && (
        <div className="space-y-3 animate-slide-up">
          <p className="text-sm text-on-surface-variant mb-2">
            Host manages the list. Any signed-in member can claim an item.
          </p>
          ...
          {event.isHost && (
            <GlassCard className="p-4 space-y-3">
              <input
                ...
                onChange={(event) =>
                  setBringForm((current) => ({ ...current, label: event.target.value }))
                }
              />
              <input
                className="input-field"
                ...
                onChange={(event) =>
                  setBringForm((current) => ({ ...current, notes: event.target.value }))
                }
              />
              <button
                className="btn-secondary"
          ...
          {bringItems.length === 0 ? (
            ...
            bringItems.map((item) => (
              <GlassCard key={item._id} className="p-4 flex items-center justify-between gap-4">
                <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    onClick={() => toggleBringItemClaim({ id: item._id })}
                    type="button"
                  >
                  ...
                  {event.isHost && (
                    <button
                      className="text-xs text-error uppercase tracking-wider font-bold"
                      onClick={() => deleteBringItem({ id: item._id })}
                      type="button"
                    >
                      Remove
                    </button>
          ...
          {event.isHost && (
            <GlassCard className="p-4 space-y-3">
              <input
                ...
                onChange={(event) =>
                  setTimelineForm((current) => ({ ...current, timeLabel: event.target.value }))
                }
              />
              <input
                className="input-field"
              ...
              <button
                className="btn-secondary"
                onClick={async () => {
                  if (!timelineForm.timeLabel.trim() || !timelineForm.title.trim()) {
                    return;
                  }
            ...
            timelineItems.map((item) => (
              <div key={item._id} className="flex gap-4">
                <div className="flex flex-col items-center">
                  <button
                    className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      item.completed ? "bg-primary/20" : "bg-surface-container-high"
                    }`}
                    disabled={!event.isHost}
                    onClick={() =>
                      event.isHost &&
                  ...
                  >
                    <span
                      className={`material-symbols-outlined text-sm ${
                        item.completed ? "text-primary" : "text-on-surface-variant"
                      }`}
                    >
                      {item.icon ?? "event"}
                    </span>
                ...
                <GlassCard className="p-4 flex-1 flex items-center justify-between">
                  <div>
                    <p className="text-xs text-on-surface-variant font-bold">{item.timeLabel}</p>
